  ...
  <div class="container">
    ...
    <div class="center-board">
      <div class="action-bar">
        ...
        <el-button icon="el-icon-back" type="text" @click="goBack">
          返回
        </el-button>
...
export default {
  ...
  mounted() {
    ...
    if (formId) {
      getForm(formId).then(res => {
        ...
        if (that.drawingList.length > 0) {
          let valuesArray = that.drawingList.map(obj => obj.__config__.formId);
          that.idGlobal = Math.max(...valuesArray);
        }
        // this.activeId
        that.form = res.data
      })
  ...
  methods: {
    ...
    handleForm() {
      ...
      this.formTitle = formId ? '修改表单' : '添加表单'
    },
    // 表单重置
    reset() {
      this.form = {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.formData.fields.length <= 0) {
            return this.$message.error('表单内容不能为空！')
          }
