      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-tickets"
            v-if="scope.row.processStatus !== 'canceled'"
...
import {recallProcess, revokeProcess} from "@/api/workflow/finished";
  ...
  methods: {
    ...
    handleFlowRecord(row) {
      if (row.formType === '0') {
        ...
        if (row.formViewPath === "/work/seal/applySignView") {
          this.$router.push({
        ...
        } else if (row.processStatus === 'running' || row.processStatus === 'completed' || row.processStatus === 'terminated') {
          this.$router.push({
    ...
    handleRecall(row) {
      ...
      recallProcess(params).then(res => {
        this.$modal.msgSuccess("流程收回成功");
        this.getList();
      });
    },
    /** 导出按钮操作 */
    handleExport() {
      this.download('/flowable/workflow/process/finishedExport', {
        ...this.queryParams
