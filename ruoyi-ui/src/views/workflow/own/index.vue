  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="流程分类" prop="category">
        <el-select v-model="queryParams.category" clearable placeholder="请选择流程分类" size="small">
          <el-option
      ...
      <el-form-item label="范围数据" prop="isComplete">
        <el-radio v-model="queryParams.isComplete" :label="true">全部</el-radio>
        <el-radio v-model="queryParams.isComplete" :label="false">未完成</el-radio>
      </el-form-item>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-if="(scope.row.processStatus !== 'completed') && (scope.row.processStatus !== 'running')"
            v-hasPermi="['workflow:process:remove']"
          >删除
          </el-button>
          <el-button
...
import {resubmit} from "../../../api/workflow/process";

export default {
  name: "Own",
  dicts: ['wf_process_status','common_whether'],
  components: {},
  data() {
    return {
      // 遮罩层
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      for (let i = 0; i < selection.length; i++) {
        if(selection[i].processStatus === 'completed' || selection[i].processStatus === 'running'){
          this.single = true
          this.multiple = true
          break
        }else {
    ...
    handleAgain(row) {
      if (row.formType === '0') {
        ...
        this.$confirm('是否重新发起流程?', '提示', {
          ...
          // 重新发起流程
          resubmit({procInsId: row.procInsId, businessId: row.businessId}).then(() => {
            this.getList();
            this.$message({
              message: '重新发起成功',
              type: 'success'
    ...
    handleFlowRecord(row) {
      if (row.formType === '0') {
        ...
        if(row.formCreatePath === "/work/seal/applySignAdd"){
          this.$router.push({
            path: '/work/seal/approveSignView',
            query: {
              procInsId: row.procInsId,
              taskId: row.taskId,
              processed: false,
              ...
              signId: row.businessId
            }
          })
        } else {
          if (row.processStatus === 'running' || row.processStatus === 'completed' || row.processStatus === 'terminated') {
            this.$router.push({
              path: row.formViewPath,
              query: {
                procInsId: row.procInsId,
                formType: row.formType
              }
