<template>
  <div class="panel-tab__content">
    ...
    <el-dialog :visible.sync="modelVisible" :title="modelConfig.title" :close-on-click-modal="false" width="400px"
      ...
      <el-form :model="modelObjectForm" size="mini" label-width="90px" ref="modelObjectForm" @submit.native.prevent>
        <el-form-item :label="modelConfig.idLabel"
                      prop="id"
                      :rules="{ required: true,message: '请输入' + modelConfig.idLabel, trigger: ['blur'] }">
          <el-input v-model="modelObjectForm.id" clearable :placeholder="`请输入${modelConfig.idLabel}`"/>
        </el-form-item>
        <el-form-item :label="modelConfig.nameLabel"
                      prop="name"
                      :rules="{ required: true,message: '请输入' +modelConfig.nameLabel, trigger: ['blur'] }">
          <el-input v-model="modelObjectForm.name" clearable :placeholder="`请输入${modelConfig.nameLabel}`"/>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    addNewObject() {
      this.$refs["modelObjectForm"].validate((validateStatus) => {
        if(validateStatus){
          if (this.modelType === "message") {
