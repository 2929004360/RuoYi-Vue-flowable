    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          v-hasPermi="['workflow:process:myCopyExport']"
          @click="handleExport"
        >导出
        </el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['workflow:process:removeMyCopy']"
          >删除
          </el-button>
        </template>
      </el-table-column>
...
<script>
import {delCopy, listMyCopyProcess} from "@/api/workflow/process"
...
export default {
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('/flowable/workflow/process/myCopyExport', {
        ...this.queryParams
      }, `wf_my_copy_process_${new Date().getTime()}.xlsx`)
    },
    /**
     * 删除抄送
     */
    handleDelete(row) {
      const ids = row.copyId;
      this.$confirm('是否确认删除抄送编号为"' + ids + '"的数据项?', "警告", {
