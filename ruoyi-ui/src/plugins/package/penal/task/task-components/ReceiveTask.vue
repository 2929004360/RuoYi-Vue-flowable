<template>
  <div style="margin-top: 16px">
    ...
    <el-dialog :visible.sync="messageModelVisible" :close-on-click-modal="false" title="创建新消息" width="400px" append-to-body destroy-on-close>
      <el-form :model="newMessageForm" size="mini" ref="newMessageForm" label-width="90px" @submit.native.prevent>
        <el-form-item label="消息ID" :rules="{ required: true,message: '请输入消息ID', trigger: ['blur'] }">
          <el-input v-model="newMessageForm.id" clearable placeholder="请输入消息ID"/>
        </el-form-item>
        <el-form-item label="消息名称" :rules="{ required: true,message: '请输入消息名称', trigger: ['blur'] }">
          <el-input v-model="newMessageForm.name" clearable placeholder="请输入消息名称"/>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    createNewMessage() {
      this.$refs["newMessageForm"].validate((validateStatus) => {
        if(validateStatus){
