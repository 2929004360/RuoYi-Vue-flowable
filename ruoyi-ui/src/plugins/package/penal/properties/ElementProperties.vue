<template>
  <div class="panel-tab__content">
    ...
    <div class="element-drawer__button">
      <el-button size="mini" type="primary" icon="el-icon-plus" @click="openAttributesForm(null, -1)">添加属性
      </el-button>
    </div>
    ...
    <el-dialog :visible.sync="propertyFormModelVisible" title="属性配置" width="600px" append-to-body destroy-on-close>
      <el-form :model="propertyForm" label-width="80px" size="mini" ref="attributeFormRef" @submit.native.prevent>
        <el-form-item label="属性名：" prop="name"
          ...
          <el-input v-model="propertyForm.name" clearable placeholder="请输入属性名"/>
        </el-form-item>
        <el-form-item label="属性值：" prop="value"
                      :rules="{ required: true,message: '请输入属性值', trigger: ['blur'] }">
          <el-input v-model="propertyForm.value" clearable placeholder="请输入属性值"/>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    saveAttribute() {
      this.$refs["attributeFormRef"].validate((validateStatus) => {
        if(validateStatus){
