      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['workflow:form:edit']"
            v-if="deptIdList.includes(scope.row.deptId)"
          >修改
          </el-button>
          <el-button
            size="mini"
...
<script>
import {addForm, delForm, listForm, updateForm} from "@/api/workflow/form";
...
export default {
  ...
  methods: {
    ...
    getListDept() {
      listDept().then((res) => {
        this.deptIdList = res.data.map((item) => {
          return item.deptId
        })
      })
    },
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({path: '/process/setFrom', query: {formId: null}})
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      const formId = row.formId || this.ids
      this.$router.push({path: '/process/setFrom', query: {formId: formId}})
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有流程表单数据项?', "警告", {
        ...
        _this.download('/flowable/workflow/form/export', {
          ..._this.queryParams
