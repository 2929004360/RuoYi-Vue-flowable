<template>
  <div class="panel-tab__content">
    <el-table :data="elementListenersList" size="mini" border>
      ...
      <el-table-column label="操作" width="90px">
        <template slot-scope="{ row, $index }">
          ...
          <el-button size="mini" type="text" style="color: #ff4d4f" @click="removeListener(row, $index)">移除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 监听器 编辑/创建 部分 -->
    <el-drawer :visible.sync="listenerFormModelVisible" title="执行监听器" :size="`${width}px`" append-to-body
               destroy-on-close>
      <div class="app-container">
        <el-form size="mini" :model="listenerForm" label-width="96px" ref="listenerFormRef" @submit.native.prevent>
          <el-form-item label="事件类型" prop="event"
            ...
            <el-select v-model="listenerForm.event" placeholder="请选择事件类型">
          ...
          <el-form-item label="监听器类型" prop="listenerType"
            ...
            <el-select v-model="listenerForm.listenerType" placeholder="请选择监听器类型">
          ...
          >
            <el-input placeholder="请输入Java类" v-model="listenerForm.class" clearable/>
          </el-form-item>
          <el-form-item
          ...
          >
            <el-input placeholder="请输入表达式" v-model="listenerForm.expression" clearable/>
          </el-form-item>
          <el-form-item
          ...
          >
            <el-input placeholder="请输入代理表达式" v-model="listenerForm.delegateExpression" clearable/>
          </el-form-item>
          <template v-if="listenerForm.listenerType === 'scriptListener'">
            ...
            >
              <el-input placeholder="请填写脚本格式" v-model="listenerForm.scriptFormat" clearable/>
            </el-form-item>
            <el-form-item
              label="脚本类型"
              prop="scriptType"
            ...
            >
              <el-select v-model="listenerForm.scriptType" placeholder="请选择脚本类型">
            ...
            >
              <el-input placeholder="请填写脚本内容" v-model="listenerForm.value" clearable/>
            </el-form-item>
            <el-form-item
            ...
            >
              <el-input placeholder="请填写资源地址" v-model="listenerForm.resource" clearable/>
            </el-form-item>
          </template>
        </el-form>
        <el-divider/>
    ...
    <!-- 注入西段 编辑/创建 部分 -->
    <el-dialog title="字段配置" :visible.sync="listenerFieldFormModelVisible" width="600px" append-to-body
               destroy-on-close>
      <el-form :model="listenerFieldForm" size="mini" label-width="96px" ref="listenerFieldFormRef"
        ...
        <el-form-item label="字段名称：" prop="name"
          ...
          <el-input v-model="listenerFieldForm.name" placeholder="请输入字段名称" clearable/>
        </el-form-item>
        <el-form-item label="字段类型：" prop="fieldType"
                      :rules="{ required: true,message: '请选择字段类型', trigger: ['blur', 'change'] }">
          <el-select v-model="listenerFieldForm.fieldType" placeholder="请选择字段类型">
...
import {fieldType, initListenerForm, initListenerType, listenerType} from "./utilSelf";
...
export default {
  ...
  methods: {
    ...
    async saveListenerFiled() {
      this.$refs["listenerFieldFormRef"].validate((validateStatus)=>{
        if(validateStatus){
    ...
    async saveListenerConfig() {
      this.$refs["listenerFormRef"].validate((validateStatus) => {
        if (validateStatus) {
